...
const resumeHeuristic = new ResumeHeuristicUtil(
  (rotationService as any).storage
);
...
async function attemptPreservedResume(context: 'onInstalled' | 'onStartup') {
  try {
    ...
    const decision = await resumeHeuristic.shouldPreserveRotation({
      wasRotating,
    });
    __lastPreserveDecision = { ...decision, decidedAt: Date.now(), context };
    if (decision.preserve) {
      console.log(
        `[bg] ${context}: preserved resume (${decision.reason}, ageSeconds=${decision.ageSeconds}).`
      );
      try {
        await (rotationService as any).storage.set({
          [StorageKeys.PreservedResumeAt]: Date.now(),
        });
      } catch {}
      try {
        (rotationService as any).metrics?.recordPreservedResume({
          ageSeconds: decision.ageSeconds,
          heartbeatAt: decision.lastHeartbeatAt,
        });
      } catch {}
      ...
      console.log(
        `[bg] ${context}: not preserving (${decision.reason}); performing normal reschedule.`
      );
      await rotationService.rescheduleIfNeeded();
    }
  } catch (e) {
...
chrome.runtime.onInstalled.addListener(() => {
  void attemptPreservedResume('onInstalled');
});
...
if (typeof __E2E_TESTING__ !== 'undefined' && __E2E_TESTING__) {
  ...
  try {
    ...
    console.warn('[bg] E2E keep-alive listener failed:', e);
  }
  try {
    (self as any).__e2eApi = {
      setConfig: async (config: any) => {
        try {
          await (rotationService as any).storage.set({
            [StorageKeys.LocalConfig]: config,
          });
          return { ok: true };
        } catch (e) {
      ...
      startWithConfig: async (config: any) => {
        const diag: any = {
          step: 'startWithConfig',
          before: { isRotating: rotationService.isRotating },
          config,
        };
        try {
          await (rotationService as any).storage.set({
        ...
        try {
          diag.rotationState = await (rotationService as any).storage.get(
            StorageKeys.RotationState
          );
          diag.localConfig = await (rotationService as any).storage.get(
            StorageKeys.LocalConfig
          );
          diag.lastInitError = (rotationService as any).lastInitializationError
            ? String((rotationService as any).lastInitializationError)
            : undefined;
          diag.createTabsDiag = (rotationService as any).__lastCreateTabsDiag;
          diag.createError = (
            rotationService as any
          ).tabManager?.__lastCreateError;
        } catch {}
        return diag;
      },
      getState: async () => {
        try {
          return await (rotationService as any).storage.get(
            StorageKeys.RotationState
          );
        } catch {
          return null;
      ...
      getDiagnostics: async () => {
        const out: any = {};
        try {
          out.rotationState = await (rotationService as any).storage.get(
            StorageKeys.RotationState
          );
        } catch {}
        try {
          out.localConfig = await (rotationService as any).storage.get(
            StorageKeys.LocalConfig
          );
        } catch {}
        try {
          out.lastInitError = (rotationService as any).lastInitializationError
            ? String((rotationService as any).lastInitializationError)
            : undefined;
        } catch {}
        try {
          out.lastInitErrorMeta = (
            rotationService as any
          ).lastInitializationErrorMeta;
        } catch {}
        try {
          out.createTabsDiag = (rotationService as any).__lastCreateTabsDiag;
        } catch {}
        try {
          out.createError = (
            rotationService as any
        ...
        try {
          out.isRotating = rotationService.isRotating;
        } catch {}
        try {
          out.starting = (rotationService as any).starting;
        } catch {}
        try {
          const tabs = await chrome.tabs.query({});
          out.openTabs = tabs.map((t) => ({
            id: t.id,
            url: t.url,
            windowId: t.windowId,
          }));
        ...
        try {
          ...
          out.pagesMeta =
            svc.diagnosticsService?.buildPagesMeta?.(tabsCfg) ||
            tabsCfg.map((t: any, i: number) => ({
              index: i,
              url: t.page?.url,
              tabId: t.tabId,
      ...
      adoptTabs: async () => {
        try {
          (rotationService as any).tabManager?.adoptOwnership(
            (rotationService as any).rotationState?.tabIds || []
          );
          return { ok: true };
        } catch (e) {
          return { ok: false, error: String(e) };
      ...
      disableAutoPreserve: async () => {
        try {
          await (rotationService as any).storage.set({
            [StorageKeys.DisableAutoPreserveNextInit]: true,
          });
          return { ok: true };
        } catch (e) {
          return { ok: false, error: String(e) };
      ...
      getMetrics: async () => {
        try {
          return (
            (rotationService as any).metrics?.snapshot?.() ||
            (rotationService as any).metrics ||
            null
          );
        } catch {
          return null;
      ...
      simulateStall: async () => {
        ...
        try {
          ...
          await (rotationService as any).storage.set({
            [StorageKeys.RotationHeartbeat]: tenMinutesAgo,
          });
        } catch (e) {
          out.heartbeatError = String(e);
        }
        try {
          ...
          if (chrome.alarms) {
            ...
            await Promise.all(
              alarms
                .filter((a) => a.name === 'rotate')
                .map((a) => chrome.alarms.clear(a.name))
            );
          }
        } catch (e) {
          out.alarmClearError = String(e);
        }
        try {
          out.stateBefore = await (rotationService as any).storage.get(
            StorageKeys.RotationState
          );
        } catch {}
        return out;
      ...
      wake: async () => {
        try {
          ...
          return { ok: true, isRotating: rotationService.isRotating };
        } catch (e) {
          return { ok: false, error: String(e) };
        }
      },
      // Reconfigure watchdog intervals (test-only) and reschedule alarm soon
      configureWatchdog: async (cfg: {
        ...
        try {
          if (
            ...
            typeof cfg.graceSeconds === 'number'
          ) {
            (rotationService as any).watchdogIntervalSeconds =
              cfg.intervalSeconds;
            (rotationService as any).watchdogGraceSeconds = cfg.graceSeconds;
            // Clear existing alarm & schedule new one quickly
            try {
              await chrome.alarms.clear('rotationWatchdog');
            } catch {}
            const ms = Math.max(500, cfg.intervalSeconds * 1000);
            try {
              await (rotationService as any).scheduler.scheduleIn(
                'rotationWatchdog',
                ms
              );
            } catch {}
      ...
      getNavCounts: async () => {
        try {
          return (self as any).__e2eNavCounts || {};
        } catch {
          return {};
        }
      },
      keepAliveDiagnostics: async () => {
        try {
          ...
          return { portCount: 0, hasPort: false };
        }
      },
      ...
      advanceIndex: async () => {
        try {
          ...
          if (svc.currentIndex === 0)
            svc.rotationCycle = (svc.rotationCycle || 0) + 1;
          try {
            await svc.stateFacade?.updateIndex(svc.currentIndex);
          } catch {}
      ...
      getCurrentIndex: async () => {
        try {
          return (rotationService as any).currentIndex;
        } catch {
          return -1;
        }
      },
      ...
      triggerWatchdog: async () => {
        try {
          ...
          if (after === before) {
            try {
              (rotationService as any).currentIndex =
                ((rotationService as any).currentIndex + 1) %
                ((rotationService as any).tabsConfig?.tabs?.length || 1);
            } catch {}
            try {
              await (rotationService as any).stateFacade?.updateIndex?.(
                (rotationService as any).currentIndex
              );
            } catch {}
            return {
              ...
              index: (rotationService as any).currentIndex,
            };
          }
      ...
      simulateNavigation: async (url: string) => {
        try {
          ...
          (self as any).__e2eNavCounts[url] =
            ((self as any).__e2eNavCounts[url] || 0) + 1;
      ...
      getOrderedUrls: async () => {
        try {
          ...
          const urls = Array.isArray(cfg?.pages)
            ? cfg.pages.map((p: any) => p.url).filter((u: string) => !!u)
            : [];
          return { ok: true, urls };
        } catch (e) {
          return { ok: false, error: String(e) };
      ...
      getPreserveDecision: async () => {
        try {
          return {
            ok: true,
            decision: (self as any).__lastPreserveDecision || null,
          };
        } catch (e) {
          return { ok: false, error: String(e) };
        }
      ...
      simulateError: async (url: string) => {
        try {
          ...
          const svc: any = rotationService as any;
          const tab = svc.tabsConfig?.tabs?.find(
      ...
      getEnforceResumeAt: async () => {
        try {
          return {
            ok: true,
            enforceResumeAt: (rotationService as any).enforceResumeAt,
          };
        } catch (e) {
          return { ok: false, error: String(e) };
        }
      },
      setEnforceResumeAtInSeconds: async (secondsFromNow: number) => {
        try {
          (rotationService as any).enforceResumeAt =
      ...
      // Test-only: evaluate arbitrary code in the service worker context
      evaluate: async (code: string) => {
        try {
          // Be very careful with this in a real app; this is for testing only.
      ...
      makePageMissing: async (pageIndex: number) => {
        try {
          const svc: any = rotationService as any;
          if (!svc.tabsConfig?.tabs?.[pageIndex])
            return { ok: false, error: 'pageIndex out of range' };
      ...
      crash: async () => {
        try {
          // Prevent any scheduled rotations from firing before we crash
          if (chrome.alarms) {
          ...
          await (rotationService as any).rotationRepo?.save?.(
            ...
            (rotationService as any).currentIndex || 0
          );
          await (rotationService as any).storage.set({
      ...
      reset: async () => {
        try {
          ...
          svc.rotationState = null;
          svc.currentIndex = 0;
          svc.isRotating = false;
          svc.starting = false;
          // Reset storage state
          await svc.storage.clear();
      ...
      shuffleAndRepair: async () => {
        try {
          ...
          if (tabs.length > 2) {
            const victimIndex = Math.min(
              tabs.length - 1,
              Math.max(1, Math.floor(Math.random() * tabs.length))
            );
            tabs[victimIndex].tabId = 0;
            tabs[victimIndex].tabIdReady = false;
          }
          // Repair: enforce invariant normal then force
          try {
            await svc.enforceInvariant?.();
          } catch {}
          try {
            await svc.enforceInvariant?.(true);
          } catch {}
          ...
          const after = Array.isArray(orderedResp?.urls)
            ? orderedResp.urls
            : [];
    ...
    console.warn('[bg] E2E API setup failed:', e);
  }
}
...
try {
  if (
    typeof __E2E_TESTING__ !== 'undefined' &&
    __E2E_TESTING__ &&
    chrome.runtime?.onConnect
  ) {
    chrome.runtime.onConnect.addListener((port) => {
      if (port?.name === 'e2e-keepalive' || port?.name === 'e2e-keepAlive') {
        // accept legacy variant
        try {
          (self as any).__e2eKeepAlivePortCount =
            ((self as any).__e2eKeepAlivePortCount || 0) + 1;
        } catch {}
        ...
        port.onDisconnect.addListener(() => {
          try {
            (self as any).__e2eKeepAlivePortCount = Math.max(
              0,
              ((self as any).__e2eKeepAlivePortCount || 1) - 1
            );
          } catch {}
        });
      }
    ...
    if (message.action === 'getDiagnostics') {
      void (async () => {
        try {
          ...
          try {
            debugFlag = await (rotationService as any).storage.get(
